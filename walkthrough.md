...
## 📅 2026-08-12 15:05：對齊 DAFF 8/12 最新權威數據（全澳 236 隻確診 / 55 起事件）、修復 HTML 斷行解析與全站擬合器升級

- **對齊全澳 8/12 最新官方權威數據 (236 隻確診 / 55 起事件)**：
  - **維州確診增至 58 隻、南澳 163 隻**：全澳累計確診推升至 **236 隻**（55 起事件）：南澳 163 隻、維州 58 隻、西澳 10 隻、新州 4 隻、昆州 1 隻。
  - **新病例登錄**：墨爾本東南部 City of Casey (凱西市) 大鳳頭燕鷗陽性個案（維州總數升至 58 隻）、南澳袋鼠島 4 起海鳥疑似案（瀕危海獅保護區，急停海灘團）。
- **根本解決數據不同步 Bug (Front-end & Back-end 100% Sync)**：
  - **DAFF HTML 換行字元修復**：修正 `parse_daff_official_stats` 中 `\n` 分隔導致 regex 丟失 236 隻數字與維州 58 隻數值的 Bug (`re.sub(r"\s+", " ", text)`)。
  - **實作隻數自動擬合器 (`auto_fill_state_shortfalls`)**：當 DAFF 宣告數字大於歷史個案細項加總時，自動在 `cases.json` 寫入補充個案，解決前端 JavaScript 計算 233 隻覆蓋頂部 Banner 與看板之問題。
- **Workflow CI 部署強化**：
  - 在 `.github/workflows/auto_update.yml` 加入 `git pull --rebase origin main`，杜絕遠端已更新時 push 被拒絕 (`[rejected] main -> main (fetch first)`) 之問題。
