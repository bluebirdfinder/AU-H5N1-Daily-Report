...
2. **智慧權威數據解析與各州確診天花板防護罩 (State Ceiling Guard)**：直接從澳洲聯邦農業部 (DAFF) 官網與 `cases.json` 數據庫解析權威確診隻數與事件數（截至目前權威對齊：**231 隻確診 / 55 起事件**），並內建「各州確診天花板防護罩 (`enforce_official_state_ceilings`)」，自動限制新聞 RSS 的過度重複新增，確保 cases.json 累加與前端面板 100% 永遠精確對齊 DAFF 官方數字。
...
* **`h5n1.py`**：自動爬取官方與新聞 RSS，結合 `curl_cffi`、Playwright 截圖與 Gemini Vision AI 視覺識別、自動定位新地點、執行權威對帳與 `max_id` 安全編號分配，並編譯輸出 `index.html` 的 Python 核心引擎。
...
* **`task.md`**：任務排程與完成度檢核表。
* **`更新說明`**：最後檔案同步日期更新於 2026-08-11 10:30 (完成 `curl_cffi` 擬真 TLS 握手降級、Playwright `--disable-http2` 防護、Gemini Vision API 多模型/429 額度重試降級、截圖零殘留自動清理，以及 2026-08-10 下午 1:00 AEST DAFF 最新官方權威數據對齊：全澳累計 231 隻確診 / 55 起事件，南澳 163 隻確診與 51 例待複驗疑似病例對齊)。
