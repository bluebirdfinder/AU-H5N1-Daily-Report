...
### v2.10.1 (2026-09-16)
- ✅ **`EBIRD_API_KEY` 已補設並驗證**：真實環境確認 eBird API 抓取成功（151–158 筆新鮮觀測），v2.10.0 記錄的候鳥數字凍結問題解除。
...
### v2.10.3 (2026-09-17)
- 🗂️ **本機工作環境重建為 git repo**：使用者將 repo 下載為裸資料夾後，補上 `git init` + 接上 GitHub remote，確認本機檔案與遠端僅有抓取時間戳記差異。
- 🛡️ **ALA 前端補上防禦性綁定**：抓取端本身在真實環境仍被 WAF 擋（195 字元回應，與 v2.10.2 現象一致），非本次修復範圍；改為抓取失敗時也會寫出安全的空殼 `ala_bird_data.json`/`assets/js/ala_bird_data.js`，`risk_assessment.html`/`_en.html` 加上對應地圖圖層渲染，資料可用時自動顯示、不可用時安全隱藏，不再有前端 `<script>` 404 風險。
- 🛰️ **Movebank 排序修正真實環境二次驗證 + 新增靜默失敗診斷**：真實 log 直接證實 v2.10.2 的排序修正生效（使用者驗證研究排名第一）；同時發現一次「全站 0 個 study」的異常且原本毫無診斷輸出，已補上原始回應長度/內容的診斷 log 供下次追查。詳見 `CLAUDE.md`。
