...
2. **智慧權威數據解析與全自動隻數擬合對齊 (Auto-Fill & State Ceiling Guard)**：直接從澳洲聯邦農業部 (DAFF) 官網與 `cases.json` 數據庫解析權威確診隻數與事件數（截至 2026-08-12 最新對齊：**236 隻確診 / 55 起事件**），並內建「各州確診天花板防護罩 (`enforce_official_state_ceilings`)」與「數據庫隻數自動擬合器 (`auto_fill_state_shortfalls`)」，確保 `cases.json` 加總與頂部 Banner、數據看板、地圖、表格與趨勢圖 100% 永遠精確對齊 DAFF 官方數字。
3. **Gemini LLM / Vision AI 雙重情報擷取引擎**：當政府官網 HTML 因阻擋或防火牆無法直接爬取時，系統會自動使用 Playwright 專門擷取黃底純數據區塊 (.callout)，最後呼叫 **Gemini Vision API**（搭配 `gemini-2.5-flash` / `gemini-2.0-flash` 多模型 429 重試）高對比精確辨識最新確診數據；並配合 **Gemini Flash LLM 文字情報提取** 解析自由文本新聞（如墨爾本 City of Casey 案例、袋鼠島海獅棲息地 4 起疑似案、菲利普島小企鵝疫苗計畫），實現全網頁 100% 自動化即時更新。
...
8. **自適應動態參考文獻庫**：底部的參考資料（References）完全動態生成，會根據當前數據庫中病例所分布的省份（如西澳 WA、維多利亞州 VIC、南澳 SA），自動追加該省政府農業廳的官方監控網址。
...
* **`.github/workflows/auto_update.yml`**：GitHub Actions 定時自動化工作流設定檔（安裝 `curl_cffi` 與 Playwright，每天定時執行 2 次，自動 commit 與 push 最新 `index.html` 與 `cases.json`）。
* **`index.html`**：編譯後生成的正式報告網頁。
* **`live_page.html`** / **`live_page_utf8.html`**：同步生成之線上備用部署網頁。
* **`walkthrough.md`**：專案開發與 Bug 修正軌跡日誌。
* **`task.md`**：任務排程與完成度檢核表。
* **`更新說明`**：最後檔案同步日期更新於 2026-08-12 15:35（完成 DAFF 官方權威數據 direct UI 解耦注入 `window.OFFICIAL_STATS`、修復天花板防護罩修剪 Bug、實現全站數據擬合器 `auto_fill_state_shortfalls`，對齊 DAFF 2026-08-12 最新官方權威數據：全澳累計 **236 隻確診 / 55 起事件**：南澳 163 隻、維州 58 隻、西澳 10 隻、新州 4 隻、昆州 1 隻。頂部 Banner、數據看板、地圖、表格、趨勢圖與文字摘要 100% 精確同步）。
