...
2. **智慧自動對帳與盲區自動補齊機制 (Reconciliation Engine)**：當政府僅發布總數而不釋出具體海岸/鎮名座標時，系統會自動比對官方各州發布目標（如 SA: 42 例、VIC: 7 例），並自動在該州沿海生成**「官方確診/未公布細節」**的盲區案例節點，確保網頁數據與地圖 100% 對齊官方總確診數（全澳 62 例）。
3. **AI 智慧地理定位與多段退避 (Nominatim API & Cloudflare Proxy)**：當新聞或官網出現全新疫情地點時，程式會自動使用 OpenStreetMap 地理編碼 API 查詢其精確 GPS 經緯度，並實作多段退避機制（自動剔除海灣沙灘等字尾）防止定位失敗，自動在網頁地圖上標示新病例。
...
* **`task.md`**：任務排程與完成度檢核表。
* **`更新說明`**：最後檔案同步日期更新於 2026-08-03 10:55 (數據與 DAFF / PIRSA / 維州農業局官網 100% 精確對齊：全澳累計確診 62 例 [南澳 42 例、西澳 10 例、維州 7 例、新州 2 例、昆士蘭 1 例]；導入智慧對帳與盲區補齊機制，確保資料庫與對帳對齊；新增多重跨域 Web Proxy 陣列及動態摘要語氣切換)。
