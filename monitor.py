...
def load_status():
    ...
    return {
        ...
        "au_daff_last_update_str": "",
        "au_daff_total_detections": 0,
        "au_daff_state_counts": {}
    }
...
def get_au_daff_vision_summary(image_path):
    print("正在向 Gemini 請求澳洲 DAFF 看板截圖視覺分析...")
    with open(image_path, "rb") as img_file:
        img_base64 = base64.b64encode(img_file.read()).decode("utf-8")
        
    prompt = (
        "您是一位生物安全專家。請分析這張來自澳洲農業、漁業及林業部 (DAFF) 的 H5 禽流感 (Bird Flu) 最新各州疫情看板截圖。\n"
        "請返回一個 JSON 物件，包含以下四個欄位（欄位名稱請務必精確相符）：\n"
        "1. \"summary_html\": 繁體中文（台灣用語）排版的疫情統計敘述。包含最新資料發布時間（如 \"1.00pm AEST, 10 August 2026\"）、澳洲全國野生鳥類 H5 確診總數、各州（WA、SA、NSW、QLD、VIC 等）個案數量分佈。並**特別重點評估新南威爾斯州（NSW）目前情況與對寵物食品/養殖業出口的潛在風險**。\n"
        "格式請「僅使用」以下 Telegram 支援的 HTML 標籤： <b>, <i>, <u>, <s>, <a href=\"...\">, <code>, <pre>。\n"
        "請絕對「不能使用」 <br>, <p>, <h1>, <h2>, <h3>, <ul>, <li>, <div> 等不支援的標籤！\n"
        "換行請直接使用標準換行字元 (\\n)，列表項目請直接在行首使用 '-' 或 '•' 字符來代表。\n"
        "2. \"last_update_str\": 看板中發布的時間字串（例如 \"1.00pm AEST, 10 August 2026\"）。\n"
        "3. \"total_detections\": 全國確診總數（必須是整數，例如 231）。\n"
        "4. \"state_counts\": 各州確診數量的 JSON 物件（例如 {\"WA\": 10, \"SA\": 163, \"NSW\": 4, \"QLD\": 1, \"VIC\": 53}）。"
    )
...
def run():
    ...
    with sync_playwright() as p:
        ...
        # 抹去 Playwright 自動化特徵，全面模擬真實人類 Chrome 瀏覽器
        ...
        # ==========================================
        # 2.5. 🇦🇺 澳洲 DAFF 官方 H5N1 統計數據看板監控 (截圖 + Vision AI / 備援 AI 摘要)
        # ==========================================
        try:
            print("正在前往澳洲 DAFF 官方 H5N1 網頁...")
            au_daff_url = "https://www.agriculture.gov.au/campaigns/birdflu"
            
            # 優先嘗試 pre-fetch HTML 以預防 Playwright goto 超時或 HTTP2 連線中斷
            html_content = None
            try:
                r_fetch = requests.get(au_daff_url, headers={'User-Agent': user_agent}, timeout=20, verify=False)
                if r_fetch.status_code == 200 and len(r_fetch.text) > 10000:
                    html_content = r_fetch.text
            except Exception as req_err:
                print(f"DAFF HTML 預先抓取提示: {req_err}")

            nav_success = False
            try:
                page.goto(au_daff_url, wait_until="domcontentloaded", timeout=45000)
                page.wait_for_selector("#state_stats", timeout=20000)
                nav_success = True
            except Exception as goto_err:
                print(f"Playwright goto DAFF 提示: {goto_err}")
                
            if not nav_success and html_content:
                try:
                    print("Playwright 連線 DAFF 逾時，改用 pre-fetched HTML 進行渲染...")
                    page.set_content(html_content, wait_until="domcontentloaded")
                    nav_success = True
                except Exception as set_err:
                    print(f"set_content 渲染提示: {set_err}")

            au_screenshot_path = "au_daff_stats.png"
            ...
            if nav_success:
                try:
                    ...
                    # 隱藏頂部固定選單列 (Header) 及中間導覽列 (.sub-nav)，實現無 bar 純黃底截圖
                    page.evaluate("""() => {
                    ...
                    # 雙重黃底區塊精確定位 (起點: 'Report signs of bird flu' 頂部; 終點: 'human health remains low' 底部)
                    ...
                    if start_box and end_box:
                        ...
                        page.screenshot(path=au_screenshot_path, clip=clip_rect)
                        print(f"已成功依新版 DAFF 黃底起點與終點精確截取全數據區塊至 {au_screenshot_path}")
                        has_screenshot = True
                    else:
                        # 備援：單獨截取黃底統計數據區塊
                        state_stats_elem = page.locator("#event_reporting, #infographics, #state_stats .callout, #state_stats").first
                        if state_stats_elem.count() > 0:
                            state_stats_elem.screenshot(path=au_screenshot_path)
                            print(f"已成功截取澳洲 DAFF 單一黃底純數據區塊至 {au_screenshot_path}")
                            has_screenshot = True
                except Exception as eval_err:
                    print(f"Playwright 截圖處理提示: {eval_err}")
                    try:
                        state_stats_elem = page.locator("#state_stats .callout, #state_stats").first
                        if state_stats_elem.count() > 0:
                            state_stats_elem.screenshot(path=au_screenshot_path)
                            has_screenshot = True
                    except Exception:
            ...
            if has_screenshot:
                au_vision_data = get_au_daff_vision_summary(au_screenshot_path)
                
                au_last_update = au_vision_data.get("last_update_str", "")
                au_total_detections = au_vision_data.get("total_detections", 0)
                au_state_counts = au_vision_data.get("state_counts", {})
                au_summary_html = au_vision_data.get("summary_html", "")
                
                print(f"澳洲 DAFF 最新數據：更新時間={au_last_update}, 全國總數={au_total_detections}, 各州={au_state_counts}")
                
                has_changed = (
                    not status.get("au_daff_last_update_str") or
                    au_last_update != status.get("au_daff_last_update_str") or
                    au_total_detections != status.get("au_daff_total_detections") or
                    au_state_counts != status.get("au_daff_state_counts")
                )
                
                if has_changed:
                    caption = (
                        f"🇦🇺 <b>澳洲 DAFF 官方 H5N1 各州疫情數據更新</b>\n"
                        f"資料來源：<a href='https://www.agriculture.gov.au/campaigns/birdflu'>DAFF Bird Flu Campaign</a>\n\n"
                        f"{au_summary_html}"
                    )
                    send_telegram_photo(au_screenshot_path, caption)
                    
                    status["au_daff_last_update_str"] = au_last_update
                    status["au_daff_total_detections"] = au_total_detections
                    status["au_daff_state_counts"] = au_state_counts
                    updated = True
                else:
                    print("澳洲 DAFF 官方數據無變動，跳過發送。")
            else:
                # 備援通道：當 Playwright 在雲端機房被阻擋或超時時，採用 Requests 靜態數據抓取 AI 摘要發送
                print("啟動 🇦🇺 澳洲 DAFF 靜態/代理備援抓取通道...")
                daff_html_text = ""
                try:
                    r_res = requests.get(au_daff_url, headers={'User-Agent': user_agent}, timeout=20, verify=not LOCAL_DEV)
                    if r_res.status_code == 200:
                        r_soup = BeautifulSoup(r_res.text, 'html.parser')
                        st_div = r_soup.find(id="state_stats")
                        if st_div:
                            daff_html_text = st_div.get_text(separator="\n", strip=True)
                        else:
                            daff_html_text = r_soup.get_text(separator="\n", strip=True)
                except Exception as r_err:
                    print(f"DAFF direct requests 備援提示: {r_err}")
                        
                if daff_html_text and len(daff_html_text) > 100:
                    summary = get_ai_summary("澳洲 DAFF 官方 H5N1 各州疫情數據看板", daff_html_text)
                    message = (
                        f"🇦🇺 <b>澳洲 DAFF 官方 H5N1 各州疫情數據更新 (文字備援模式)</b>\n"
                        f"資料來源：<a href='https://www.agriculture.gov.au/campaigns/birdflu'>DAFF Bird Flu Campaign</a>\n\n"
                        f"{summary}"
                    )
                    send_telegram_message(message)
                    status["au_daff_last_update_str"] = "checked"
                    updated = True
                    
        except Exception as e:
            print(f"檢查澳洲 DAFF 失敗: {e}")
